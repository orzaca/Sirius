...
$sql = "SELECT p.id, COALESCE(up.content, p.content) AS content FROM pagina5 p  
        LEFT JOIN user_pagina5 up ON p.id = up.pagina5_id AND up.user_id = :userId";
...
<body>
    ...
    <div class="content-wrapper">
        ...
        foreach ($paragraphs as $para) {
            echo '<div class="paragraph-box" id="para5' . $para['id'] . '">'; // Cambiado a para2
            echo '<p>' . htmlspecialchars($para['content']) . '</p>';
            echo '<button onclick="copyText(\'para5' . $para['id'] . '\')">Copiar</button>';// Cambiado a para2
            echo '<button onclick="openEditModal5(' . $para['id'] . ')">Modificar</button>';
    ...
    <div id="editModal5"> <!-- Cambiado a editModal3 -->
        <div class="modal-content">
            <span class="close-btn" onclick="closeEditModal()">&times;</span>
            <h2>Editar Párrafo</h2>
            <textarea id="editText5" rows="4" style="width: 100%;"></textarea> <!-- Cambiado a editText3 -->
            <button id="saveChanges5" onclick="saveParagraph()">Guardar</button> <!-- Cambiado a saveChanges3  -->
        </div>
    </div>
